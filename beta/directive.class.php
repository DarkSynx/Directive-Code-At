...
class directive {
	...
	private function bop($find='',$fdb=0,$deb='', $fin='',$debx='<?php ',$endx=' ?>',$bdeb1='(',$bfin1=')',$bdeb2='{',$bfin2='}',$masque1='[%1]',$masque2='[%2]',$exp=TRUE,$b=0, $m2='', $m3='', $c=0): bool 
	{		
			global $data, $segment;
			...
			while(--$fdb >= 0) {
				...
				if($b !== false ) {
					...
					if($data[$bs] == '(') {
						list($m2,$bs) = $this->shearch( strpos($data,')',$bs), $bs, $dd - $bs, '(', ')'); 
					}
					echo '-2-' .  PHP_EOL;
					var_dump($m2);
					
					if($data[$bs] == '{') {
						list($m3,$e) = $this->shearch( strpos($data,'}',$bs), $bs, $dd - $bs, '{', '}');
					}
					echo '-3-' .  PHP_EOL;
					var_dump($m3);
					...
					/*
						...
						switch(true){
							case ($data[$bs] . $data[$bs + 1]) == "$bdeb1$bfin1": 
								$c = $bs + 2; //()
							break;
							case ($data[$bs] . $data[$bs + 1] . $data[$bs+2]) == " $bdeb1$bfin1":
								$c = $bs + 3; //_()
							break;
							case ($data[$bs] == $bdeb1 && $data[$bs+1] != $bfin1):
								$c = strpos($data,$bfin1,$bs); //(?
									$c = ($data[$bs+1] == $bdeb1) ? $bs+1 : $c ;
									list($m2,$c) = $this->shearch( $c, $bs, $dd - $b, $bdeb1, $bfin1); 
									$node2 = false;	
							break;
							case ($data[$bs] . $data[$bs + 1] == " $bdeb1" && $data[$bs+2] != $bfin1):
								 $bs = $bs + 1; //_( ?
								 $c = strpos($data,$bfin1,$bs);
							...
							default:
								...
								if( ($bdeb2) && ( ($data[$c] == $bdeb2) || ($data[$c+1] == $bdeb2) ) ){ 
											...
											list($m3,$e) = $this->shearch( $e, $c, $dd - $c, $bdeb2, $bfin2);

									if( ($masque2) && ($exp === false) && ($node2 === false)) { $m3 = "echo <<<END2 \r\n$m3\r\nEND2;\r\n"; }
